...
import { Product, DevicePoint, Room, Solution, VectorFloorPlanData, SmartBrand, DesignMode, SolutionTemplate, DesignStyle } from '../types';
...
import { 
  ...
  DollarSign, Zap, HardDrive, MousePointer2, Grid, Filter, Check, Palette
} from 'lucide-react';
...
interface DesignerProps {
  ...
  onSave: (devices: DevicePoint[], rooms: Room[], originalId?: string, vectorData?: VectorFloorPlanData, style?: DesignStyle) => void;
}

type Step = 'brand' | 'style' | 'mode' | 'template-select' | 'upload' | 'design';
      ...
      <div className="max-w-5xl w-full space-y-12 z-10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <button key={mode.id} onClick={() => { setDesignMode(mode.id as DesignMode); if (mode.id === DesignMode.TEMPLATE) setStep('template-select'); else setStep('upload'); }} className="group relative bg-white/5 border border-white/10 p-10 rounded-[3.5rem] hover:bg-white/10 transition-all text-left overflow-hidden h-[320px] flex flex-col justify-between">
    ...
    <div className="h-full flex flex-col items-center justify-center p-12 bg-slate-50 overflow-y-auto animate-in fade-in duration-500">
      <div className="max-w-6xl w-full space-y-12">
        <div className="flex items-center justify-between">
           <button onClick={() => setStep('mode')} className="p-4 bg-white border rounded-2xl text-slate-400 hover:text-slate-900 transition-all flex items-center space-x-2"><ChevronLeft size={20} /><span className="font-bold">返回</span></button>
  ...
  const renderUploadStep = () => (
    <div className="h-full flex flex-col items-center justify-center p-12 bg-slate-50 animate-in fade-in duration-500">
      <div className="max-w-4xl w-full text-center space-y-8">
        ...
        <div className="flex justify-center space-x-4"><button onClick={() => setStep(designMode === DesignMode.TEMPLATE ? 'template-select' : 'mode')} className="px-10 py-4 font-bold text-slate-500 hover:bg-white rounded-2xl transition-all">返回修改设计路径</button></div>
      </div>
    </div>
  );
  ...
  const renderDesignStep = () => (
    <div className="flex h-full animate-in fade-in duration-700 bg-white overflow-hidden">
      <div className="flex-1 flex flex-col relative bg-slate-900">
         ...
         <div className="absolute bottom-8 left-8 z-20 flex items-center space-x-4">
            <div className="bg-slate-950/90 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-[2rem] flex items-center space-x-12 shadow-2xl text-white">
               ...
               <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Decor Style</p><p className="font-bold text-blue-400">{designStyle}</p></div>
               <div className="w-px h-8 bg-white/10" />
               <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Method</p><p className="font-bold">{designMode}</p></div>
            </div>
         </div>
      </div>
      <div className="w-[480px] border-l flex flex-col bg-slate-50 z-30">
         ...
         <div className="p-8 bg-white border-t space-y-6"><div className="flex items-center justify-between"><p className="text-xs font-black text-slate-400 uppercase tracking-widest">方案预估总额</p><p className="text-4xl font-black text-blue-600">${bom.reduce((acc, i) => acc + (i.product.price * i.quantity), 0).toLocaleString()}</p></div><div className="grid grid-cols-2 gap-4"><button onClick={() => onSave(devices, rooms, initialSolution?.id, vectorData!, designStyle)} className="flex items-center justify-center space-x-2 py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all shadow-xl"><Save size={18} /><span>完成设计</span></button><button className="flex items-center justify-center space-x-2 py-4 bg-slate-100 text-slate-900 rounded-2xl font-bold hover:bg-slate-200 transition-all"><Share2 size={18} /><span>分享预览</span></button></div></div>
      </div>
    </div>
  );
