...
import { Product, DevicePoint, Room, Solution, VectorFloorPlanData, SmartBrand, DesignMode } from '../types';
import { CATEGORY_ICONS, SMART_BRANDS } from '../constants';
import { 
  ...
  DollarSign, Zap, HardDrive, MousePointer2
} from 'lucide-react';
...
type Step = 'brand' | 'mode' | 'upload' | 'design';
...
const FloorPlanDesigner: React.FC<DesignerProps> = ({ products, onSave, initialSolution }) => {
  ...
  const processAlgorithm = () => {
    ...
    setTimeout(async () => {
      ...
      // If AI or Template mode, pre-populate devices
      if (designMode !== DesignMode.CUSTOM) {
        try {
    ...
    <div className="h-full flex flex-col items-center justify-center p-12 bg-slate-950 text-white animate-in fade-in slide-in-from-right duration-500 overflow-hidden relative">
      ...
      <div className="max-w-5xl w-full space-y-12 z-10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { id: DesignMode.AI, title: lang === 'zh' ? 'AI 自动生成' : 'AI Generation', desc: '基于空间布局与生活习惯智能排布', icon: <Sparkles className="text-blue-400" />, color: 'from-blue-600 to-indigo-600' },
              { id: DesignMode.TEMPLATE, title: lang === 'zh' ? '方案模板生成' : 'Expert Template', desc: '应用行业标准的高端智能方案模板', icon: <Layout className="text-purple-400" />, color: 'from-purple-600 to-pink-600' },
              // Added missing MousePointer2 import from lucide-react
              { id: DesignMode.CUSTOM, title: lang === 'zh' ? '设计师自定义' : 'Manual Custom', icon: <MousePointer2 className="text-emerald-400" />, desc: '从零开始，由您主导每一个细节' }
            ].map(mode => (
              <button 
                key={mode.id}
                onClick={() => { setDesignMode(mode.id as DesignMode); setStep('upload'); }}
    ...
    <div className="h-full flex flex-col items-center justify-center p-12 bg-slate-50 animate-in fade-in duration-500">
      <div className="max-w-4xl w-full text-center space-y-8">
          ...
          {isLoading ? (
            ...
            <div className="text-center space-y-6">
               ...
               <div>
                  ...
                  <p className="text-slate-400 font-bold uppercase tracking-widest text-xs mt-2">Structure detection will follow upload</p>
               </div>
            </div>
          )}
        ...
        <div className="flex justify-center space-x-4">
           <button onClick={() => setStep('mode')} className="px-10 py-4 font-bold text-slate-500 hover:bg-white rounded-2xl transition-all">返回修改设计路径</button>
        </div>
      </div>
    </div>
  );
  ...
  const renderDesignStep = () => (
    <div className="flex h-full animate-in fade-in duration-700 bg-white overflow-hidden">
      {/* Sidebar: BOM List (Right side based on prompt) */}
      <div className="flex-1 flex flex-col relative bg-slate-900">
         ...
         <div className="flex-1">
            {activeView === '3d' && vectorData ? (
               ...
               <VectorFloorPlan 
                  ...
                  devices={devices}
                  products={products}
                  onAddDevice={(pId, x, y, rName) => setDevices([...devices, { id: Math.random().toString(36).substr(2, 9), productId: pId, x, y, roomName: rName, status: 'on' }])}
         ...
         <div className="absolute bottom-8 left-8 z-20 flex items-center space-x-4">
            <div className="bg-slate-950/90 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-[2rem] flex items-center space-x-12 shadow-2xl text-white">
               ...
               <div className="w-px h-8 bg-white/10" />
               <div>
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Total Power</p>
                  <p className="font-bold text-blue-400">1.2 kW</p>
               </div>
            </div>
         </div>
      </div>

      {/* Right Side: Product Library & Real-time BOM */}
      <div className="w-[480px] border-l flex flex-col bg-slate-50 z-30">
         <div className="p-8 border-b bg-white flex items-center justify-between">
            <div className="flex items-center space-x-4">
               ...
               <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight">{lang === 'zh' ? '方案清单' : 'Project BOM'}</h3>
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{devices.length} Devices Placed</p>
               </div>
            </div>
            <button 
              onClick={() => onSave(devices, rooms, initialSolution?.id, vectorData!)}
              className="flex items-center space-x-2 px-8 py-3 bg-blue-600 text-white rounded-2xl font-bold shadow-xl hover:bg-blue-700 transition-all hover:scale-105"
            >
               <Save size={18} />
               <span>{lang === 'zh' ? '生成方案' : 'Generate'}</span>
            </button>
         </div>
         ...
         <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
            {/* BOM Sections */}
            <div className="space-y-4">
               <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">实时物料清单汇总</h4>
               <div className="space-y-2">
                    ...
                    <div key={item.product.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between group">
                       <div className="flex items-center space-x-4">
                          ...
                          <div>
                             ...
                             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Unit: ${item.product.price}</p>
                          </div>
                       </div>
                       <div className="flex items-center space-x-4">
                  ...
                  {bom.length === 0 && (
                    <div className="py-12 text-center bg-white rounded-[2.5rem] border border-dashed border-slate-200">
                       ...
                       <p className="text-slate-400 text-xs font-bold uppercase">清单为空 · 请在左侧布置点位</p>
                    </div>
                  )}
               </div>
            </div>

            {/* Manual Library for Custom Design */}
            {designMode === DesignMode.CUSTOM && (
               <div className="space-y-4 pt-4 border-t">
                  <div className="flex items-center justify-between px-2">
                     <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{selectedBrand} 设备库</h4>
                     <Search size={14} className="text-slate-300" />
                  </div>
                  <div className="grid grid-cols-1 gap-3">
                     {products.filter(p => p.brand.toLowerCase().includes(selectedBrand?.toLowerCase() || '')).map(p => (
                        <div 
                          key={p.id}
                          onClick={() => {
                             // Find a logical place to drop (e.g., center)
                             const x = vectorData?.width ? vectorData.width / 2 : 250;
                             const y = vectorData?.height ? vectorData.height / 2 : 180;
                             setDevices([...devices, { id: Math.random().toString(36).substr(2, 9), productId: p.id, x, y, roomName: 'Manual', status: 'on' }]);
                          }}
                          className="bg-slate-100/50 p-4 rounded-2xl border border-transparent hover:border-blue-300 hover:bg-white transition-all cursor-pointer group"
                        >
                           <div className="flex items-center space-x-4">
         ...
         <div className="p-8 bg-white border-t space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
               <button className="flex items-center justify-center space-x-2 py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all">
                  <Share2 size={18} />
                  <span>分享方案</span>
               </button>
